...
class OrderService
{
    ...
    public function calculateShippingCost(int $userId, int $addressId): array
    {
        ...
        foreach ($groupedByVendor as $vendorId => $items) {
            ...
            $selectedVendors[] = [
                ...
                'branch_lat' => (float) ($branch->latitude ?? 0),
                'branch_lon' => (float) ($branch->longitude ?? 0),
        ...
        // Customer shipping: price_per_km * (branch1->branch2 + branch2->branch3 + ... + lastBranch->customerAddress)
        $pricePerKm = (float) setting('shipping_price_per_km', 5);
        $count = count($selectedVendors);

        for ($i = 0; $i < $count; $i++) {
            $from = $selectedVendors[$i];

            if ($i < $count - 1) {
                $to = $selectedVendors[$i + 1];
                $distanceKm = $this->distanceKm(
                    (float) $from['branch_lat'],
                    (float) $from['branch_lon'],
                    (float) $to['branch_lat'],
                    (float) $to['branch_lon']
                );
            } else {
            ...
            $shippingCost = round($distanceKm * $pricePerKm, 0);
    ...
    public function createOrder(int $userId, array $data): Order
    {
        return DB::transaction(function () use ($userId, $data) {
            ...
            foreach ($vendorData as $vendorId => $vData) {
                ...
                $vendorData[$vendorId]['branch_lat'] = (float) ($branch->latitude ?? 0);
                $vendorData[$vendorId]['branch_lon'] = (float) ($branch->longitude ?? 0);
            }

            // 6. Calculate Customer Shipping Costs along the chain of selected vendor branches
            // Total = price_per_km * (branch1->branch2 + ... + lastBranch->customerAddress)
            $pricePerKm = (float) setting('shipping_price_per_km', 5);
            $totalShipping = 0.0;
            $orderedVendorIds = array_keys($vendorData); // preserve insertion order from cart grouping
            $count = count($orderedVendorIds);

            for ($i = 0; $i < $count; $i++) {
                $vendorId = $orderedVendorIds[$i];

                $fromLat = (float) ($vendorData[$vendorId]['branch_lat'] ?? 0);
                $fromLon = (float) ($vendorData[$vendorId]['branch_lon'] ?? 0);

                if ($i < $count - 1) {
                    $nextVendorId = $orderedVendorIds[$i + 1];
                    $toLat = (float) ($vendorData[$nextVendorId]['branch_lat'] ?? 0);
                    $toLon = (float) ($vendorData[$nextVendorId]['branch_lon'] ?? 0);
                } else {
                    $toLat = (float) $address->latitude;
                    $toLon = (float) $address->longitude;
                }

                $distanceKm = $this->distanceKm($fromLat, $fromLon, $toLat, $toLon);
                $shippingCost = round($distanceKm * $pricePerKm, 0);

                $vendorData[$vendorId]['shipping_cost'] = $shippingCost;
                $totalShipping += $shippingCost;
            }
    ...
    private function calculateVendorShipping(int $vendorId, Branch $branch, Address $address, float $vendorSubtotal): float
    {
        ...
        $shipping = round($distance * $costPerKm, 0);
